...
interface IPlaced extends IPlacement {
    color: Stone;
    time: number;
}
...
export const GAME_INFO = 'room-info';
...
export interface ServerToClientEvents {
    ...
    [PLACED]: (data: IPlaced) => void
    [TURN]: (stone: Stone) => void
    [GAME_INFO]: (room: ISocketGame) => void // 두 플레이어에 대한 정보 (닉네임) 와 방의 아이디를 포함한다. 누가 흑돌인지 (누가 게임을 시작해야 하는지) 를 표현한다.
